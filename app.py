...
def solve_puzzle():
    try:
        ...
        if algorithm == 'bfs':
            solution_node, _, nodes_expanded_count , search_depth= bfs(initial_state)
            
        elif algorithm == 'dfs':
            solution_node, _, nodes_expanded_count , search_depth= dfs(initial_state)
            
        elif algorithm == 'iddfs':
            limit = 30 # Default depth limit for IDDFS
            solution_node, _, nodes_expanded_count,search_depth = iddfs(initial_state, limit)

        elif algorithm == 'a_star_manhattan':
            solution_node, _, nodes_expanded_count ,search_depth= A_star(initial_state, h_name="Manhattan")
            
        elif algorithm == 'a_star_euclidean':
            solution_node, _, nodes_expanded_count ,search_depth = A_star(initial_state, h_name="Euclidean")
